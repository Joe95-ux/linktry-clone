  ...
  return (
    <>
     <section className='relative'>
        <img className='w-20 absolute rounded-full left-1/2 -translate-x-1/2 mt-2' src={avatar} alt='user avatar'/>
        ...
        <div  className='flex flex-col justify-center max-w-7xl m-auto md:my-5 w-full md:w-2/5'>
          <AnimatePresence>
            {links.map((link, index)=>{
              <motion.div key={index} initial={{opacity: 0, y:40}} animate={{opacity:1, y:0, transition: {delay: index * 0.1 + 0.5}}}>
